class Repair < ActiveRecord::Base
  ...
  scope :last_12_months, where("repaired_on >= ?", 12.month.ago)

  def self.graph
    Hash[ :name => "Repairs",
      :data => Hash[last_12_months.group_by_month(:repaired_on).order("month asc").sum(:cost).map {|k, v| [Date.strptime(k).strftime("%b %Y"), v]}]
    ]
  end
end
